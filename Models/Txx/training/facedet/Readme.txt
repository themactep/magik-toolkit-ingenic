...
3.网络量化与参数配置
（2个文件与参数需要时手动配置）
文件1： vision/quantize_cfg.py
IS_QUANTIZE = True     #如果非量化。请改为False。量化为True
BITA = 5               #如果8bit量化时  BITA 改为8即可  4bit量化为减少损失暂时设置为5
BITW = 5	       #与BITA值一致即可
SPECIAL_BIT = 4        #4bit 量化为4  8bit 为8
CLIP_MAX_VALUE = 3.0   #CLIP值8bit量化时建议为3.0/6.0  4bit 3.0与2.0都可以。可能依据训练数据而变。

文件2： vision/ssd/mb_tiny_fd.py
参数同上。
...
6.WIDER_val 测试集量化流程测试结果
	...
	#4bit  5 4        
	    clip3.0               clip4.0           clip2.0  
	Easy:   0.7741         Easy:   0.771      Easy:   0.7738 
	Medium: 0.6773	       Medium: 0.676      Medium: 0.6801
	Hard:   0.3808         Hard:   0.385      Hard:   0.3882
